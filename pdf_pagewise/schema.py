...
__version__ = "1.1.1"  # bump this when you update the schema
...
class PageModel(BaseModel):
    ...
    page_index: int
    text: str = ""
    tables: Optional[List[TableModel]] = Field(default_factory=list)           # accept None
    kv_fields: Optional[Dict[str, FieldValue]] = Field(default_factory=dict)   # accept None
    lang: Optional[str] = None
    ocr: bool = False
    lines: Optional[List[OCRLine]] = Field(default_factory=list)               # accept None

    # Coerce before validation
    @model_validator(mode="before")
    @classmethod
    def _coerce_nulls_before(cls, data):
        if isinstance(data, dict):
            if data.get("kv_fields") is None:
                data["kv_fields"] = {}
    ...
    # And coerce after model is built (covers edge cases like construct/copy)
