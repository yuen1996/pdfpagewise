...
def _ocr_best_of(pil_img: Image.Image) -> str:
...
def _page_text_with_ocr(pdf: pdfplumber.PDF, file_bytes: bytes, idx1: int,
                        aggressive_ocr: bool, try_ocrmypdf: bool) -> Tuple[str, bool]:
...
def _extract_tables_for_page(page: pdfplumber.page.Page, idx1: int) -> List[TableModel]:
    ...
    tables = []
    # try multiple strategies, keep unique tables by header signature
    seen = set()
