...
def _ocr_best_of(pil_img: Image.Image, *, speed_mode: bool,
...
def extract_pagewise(
    ...
    *,
    ...
    speed_mode: bool = True,  # NEW: faster defaults
    progress_cb: Optional[Callable[[Dict[str, Any]], None]] = None,
) -> CanonicalDoc:
    ...
    # (B) decide if we need OCRmyPDF; if yes, run once & reuse
    ocr_langs = os.getenv("OCR_LANGS", "chi_sim+chi_tra+eng+msa")
    ocr_pdf_obj = None
    if try_ocrmypdf:
        # sample first few pages for text length
        sample_len = 0
    ...
    # fallback function to get text quickly (text layer or pdftotext) from chosen source
    ...
    for idx1 in range(1, total_pages + 1):
        ...
        # 1) try text layer (prefer ocr_pdf_obj if available)
        txt = _text_layer(ocr_pdf_obj or base_pdf, idx1)
        # 2) if still weak, try pdftotext once
        if len(txt.strip()) < 20:
        ...
        # 3) only if still weak -> Tesseract OCR (fast path first)
        if len(txt.strip()) < 60:
    ...
    source = DocSource(filename=filename, pages=len(pages), sha256=_sha256_bytes(file_bytes), lang=lang)
