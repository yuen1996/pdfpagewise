...
def _pdftotext_page(file_bytes: bytes, page_no: int) -> str:
    """Poppler fallback: extract a single page's text with better spacing."""
...
def _page_text_with_ocr(pdf: pdfplumber.PDF, file_bytes: bytes, idx1: int) -> Tuple[str, bool]:
    """Extract text for 1-based page index. Try pdfplumber -> pdftotext -> OCR.
    ...
    # pdftotext fallback if text is very short / badly spaced
    if len(txt.strip()) < 20:
    ...
    try:
        ...
        langs = os.getenv("OCR_LANGS", "eng+msa+chi_sim+chi_tra")

        imgs = convert_from_bytes(file_bytes, dpi=dpi, first_page=idx1, last_page=idx1)
        if imgs:
            cfg = f"--psm {psm}"
            txt = pytesseract.image_to_string(imgs[0], lang=langs, config=cfg) or txt
            ocr_used = True
    except Exception:
        pass
...
def _extract_tables_for_page(page: pdfplumber.page.Page, idx1: int) -> List[TableModel]:
    ...
    for j, t in enumerate(tables, start=1):
        ...
        # CLEAN description column(s) to fix 'Bu tterm ilk' etc.
        cleaned_rows = []
        for r in rows:
            rr = [str(c) if c is not None else None for c in r]
            # Description usually at index 2; if a Rate column exists (Second Schedule),
            # description is still index 2 and rate index 3.
