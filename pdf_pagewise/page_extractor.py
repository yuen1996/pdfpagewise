...
def _ocrmypdf_full(file_bytes: bytes, langs: str) -> Optional[bytes]:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        with open(inp, "wb") as f:
            f.write(file_bytes)
        # basic options; let OCRmyPDF auto-detect needed things
        cmd = ["ocrmypdf", "--force-ocr", "--jobs", "2", "--output-type", "pdf",
...
# --- doc-level summarizer (OpenAI) ---
MAX_DOC_CHARS = 15000

def _llm_doc_summary(text: str, api_key: str, model: str = DEFAULT_MODEL) -> dict:
    """Return {"summary": str, "key_points": [str, ...]} using OpenAI (JSON)."""
    ...
    resp = client.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "user", "content":
             "Summarize the document. Respond as JSON with keys: "
             "`summary` (string, ≤200 words) and `key_points` (array of 5–8 bullets). "
             f"TEXT:\n{clipped}"}
        ],
    )
    try:
...
def extract_image(
    ...
    *,
    ...
    """Process a single image as a 1-page CanonicalDoc."""
    ...
    doc = CanonicalDoc(
        ...
        pages=[PageModel(page_index=1, text=txt, tables=[], kv_fields=kv_fields, lang=lang, ocr=True)],
    )

    # doc-level summary into meta (optional)
    if api_key:
        full_text = txt or ""
        meta = _llm_doc_summary(full_text, api_key=api_key, model=model)
        if meta:
...
def extract_pagewise(
    ...
    *,
    ...
    # --- doc-level LLM summary into meta (optional) ---
    if api_key:
