...
def _extract_tables_for_page(page: pdfplumber.page.Page, idx1: int) -> List[TableModel]:
    ...
    for j, t in enumerate(tables, start=1):
        ...
        # Only keep "real" First/Second Schedule-style tables
        # (First: 3 cols header; Second: we’ll allow, the normalizer will check for Rate column)
...
def extract_pagewise(
    ...
    *,
    ...
    # ---------- Normalized outputs ----------
    # First Schedule: explode + cleaned HS + rate_percent numeric
    first_items = normalize_first_schedule_items(pages, rate, doc_id)
    # Second Schedule: parse "Rate of tax (4)" into structured fields
...
def build_excel_sheets(doc: CanonicalDoc) -> Dict[str, "pandas.DataFrame"]:
    ...
    # Normalized First/Second Schedules
