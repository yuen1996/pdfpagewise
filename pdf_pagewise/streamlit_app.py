...
if st.button("Process (Page-by-Page)", type="primary"):
    for up in files:
        ...
        # Excel download (page-wise text/kv/tables + normalized + rules)
        xbuf = io.BytesIO()
        sheets = build_excel_sheets(doc)
        with pd.ExcelWriter(xbuf, engine="xlsxwriter") as xw:
            for sname, df in sheets.items():
                df.to_excel(xw, index=False, sheet_name=sname[:31])  # Excel limit 31 chars
        xbuf.seek(0)
        st.download_button(
        ...
        # Quick preview: Rules + normalized count (if any)
        if doc.rules:
            st.info(f"Detected rules: {doc.rules}")
        if doc.normalized and "first_schedule_items" in doc.normalized:
            st.success(f"Normalized rows: {len(doc.normalized['first_schedule_items'])}")
