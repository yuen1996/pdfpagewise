...
from pydantic import ValidationError  # NEW: cleaner validation error handling
...
if up:
    ...
    try:
        ...
        st.error(
            "Validation failed while building pages. "
            "This usually means a caller sent None where a dict/list is expected. "
            "Please update to the latest code and try again.\n\n" + str(ve)
        )
        st.stop()
    ...
    for p in doc.pages:
        with st.expander(f"Page {p.page_index}"):
            ...
            with col1:
                # UNIQUE KEY per page to avoid duplicate element IDs
            ...
            if getattr(p, "lines", None):
                ...
                for L in p.lines:
                    ...
                    line_rows.append({
                        ...
                        "text": L.text, "conf": L.confidence if hasattr(L, "confidence") else getattr(L, "conf", None),
                    })
