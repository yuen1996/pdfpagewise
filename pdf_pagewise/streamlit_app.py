...
if st.button("Process (Page-by-Page)", type="primary"):
    ...
    for up in files:
        ...
        done_pages = 0
        total_pages_est = None  # filled by callback on first event
        ...
        def cb(ev: dict):
            nonlocal done_pages, total_pages_est
            stage = ev.get("stage")
            i = ev.get("i")
            n = ev.get("n")
            if n and total_pages_est is None:
                total_pages_est = n
            ...
            if total_pages_est:
                frac = done_pages / total_pages_est
                bar.progress(min(1.0, frac))
                elapsed = time.perf_counter() - start_all
                avg = (sum(page_times) / len(page_times)) if page_times else None
                remain = (total_pages_est - done_pages) * avg if (avg and total_pages_est) else None
                if avg is not None and remain is not None:
                    eta_line.write(f"**Progress:** {done_pages}/{total_pages_est}  •  **Elapsed:** {elapsed:.1f}s  •  **ETA:** {fmt_eta(remain)}")
