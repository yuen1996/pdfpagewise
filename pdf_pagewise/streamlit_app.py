...
with st.sidebar:
    ...
    dpi = st.slider("OCR DPI", 200, 600, int(os.getenv("OCR_DPI", "300")), 50)  # default 300 for speed
    ...
    speed_mode = st.checkbox("Speed mode (fast)", value=True,
                             help="Runs OCRmyPDF once for the whole PDF, then minimal OCR passes per page.")
...
if st.button("Process (Page-by-Page)", type="primary"):
    ...
    for up in files:
        ...
        doc = extract_pagewise(
            ...
            speed_mode=speed_mode,           # NEW: fast path
            progress_cb=cb,
        )
